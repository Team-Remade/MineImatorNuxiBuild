...
public class Viewport : UiPanel
{
    ...
    private void RenderSky(Camera camera, uint width, uint height)
    {
        ...
        Mesh.GlobalSubsurfaceEnabled = p.SubsurfaceScatteringEnabled;
        Mesh.GlobalSubsurfaceHighQuality = p.SubsurfaceScatteringHighQuality;
        Mesh.GlobalSubsurfaceBlurSample = Math.Clamp(p.SubsurfaceScatteringBlurSample, 1, 64);
        Mesh.GlobalSubsurfaceStrength = Math.Clamp(p.SubsurfaceScatteringStrength, 0f, 4f);
        Mesh.GlobalSubsurfaceSharpness = Math.Clamp(p.SubsurfaceScatteringSharpness, 0f, 1f);
        Mesh.GlobalSubsurfaceDesaturation = Math.Clamp(p.SubsurfaceScatteringDesaturation, 0f, 1f);
        Mesh.GlobalSubsurfaceColorThreshold = Math.Clamp(p.SubsurfaceScatteringColorThreshold, 0f, 1f);
        Mesh.GlobalSubsurfaceHighlightSize = Math.Clamp(p.SubsurfaceScatteringHighlightSize, 0.01f, 8f);
        Mesh.GlobalSubsurfaceHighlightStrength = Math.Clamp(p.SubsurfaceScatteringHighlightStrength, 0f, 4f);
        Mesh.GlobalSubsurfaceHighlightSharpness = Math.Clamp(p.SubsurfaceScatteringHighlightSharpness, 0f, 1f);
        Mesh.GlobalSubsurfaceHighlightDesaturation = Math.Clamp(p.SubsurfaceScatteringHighlightDesaturation, 0f, 1f);
        Mesh.GlobalSubsurfaceHighlightColorThreshold = Math.Clamp(p.SubsurfaceScatteringHighlightColorThreshold, 0f, 1f);
        Mesh.GlobalSubsurfaceAbsorption = Math.Clamp(p.SubsurfaceScatteringAbsorption, 0f, 0.95f);
